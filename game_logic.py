...
def check_win(board: list[list[Symbol]], size: int) -> Symbol | str:
...
def take_turns(board: list[list[Symbol]], symbols: list[Symbol], player_name: str, player_symbol: str, computer_symbol: str) -> str | None:
    ...
    while winner == Symbol.EMPTY.value:
        ...
        if player_symbol == symbols[0]:
            ...
            if move is not None:
                row, col = move
                board[row][col] = Symbol(computer_symbol).value  # type: ignore
                player_symbol, computer_symbol = computer_symbol, player_symbol
            else:
                print("It's a tie!")
                break
        winner = check_win(board, BOARD_SIZE)  # type: ignore
    display_board(board)
